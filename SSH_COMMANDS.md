# SSH Commands to Create Azure Fix File

## Method 1: Copy-Paste Single Command (Easiest)

SSH into your VPS and run this **entire command** (it's long, but copy it all):

```bash
mkdir -p /opt/chatwoot-patches/enterprise/app/services/llm && cat > /opt/chatwoot-patches/enterprise/app/services/llm/base_open_ai_service.rb << 'ENDOFFILE'
...
mkdir -p /opt/chatwoot-patches/enterprise/app/services/llm

# Step 2: Create file with nano (easier to edit)
nano /opt/chatwoot-patches/enterprise/app/services/llm/base_open_ai_service.rb
```

Then paste the file content (from the file I modified), save with `Ctrl+X`, then `Y`, then `Enter`.
...
vi /opt/chatwoot-patches/enterprise/app/services/llm/base_open_ai_service.rb
```
...
## Method 4: Verify File Was Created
...
head -20 /opt/chatwoot-patches/enterprise/app/services/llm/base_open_ai_service.rb
```

## Recommended: Method 1

Method 1 (single command) is easiest - just copy the entire command and paste it into your SSH terminal. It will create the file with all the correct content.
